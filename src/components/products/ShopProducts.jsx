...
const ShopProducts = ({styles, products}) => {
    return (
        <div className={`w-full grid ${styles === 'grid' ? 'grid-cols-3 md-lg:grid-cols-2 md:grid-cols-2' : 'grid-cols-1 md-lg:grid-cols-2 md:grid-cols-2'} gap-3`}>
            {
                products.length > 0 ? products.map((p,i) => <div key={i} className={`flex transition-all duration-1000 hover:shadow-md hover:-translate-y-3 
            ...
            w-full gap-4 bg-white p-1 rounded-md`}>
                ...
                </div> ) : <div className='justify-start items-center md-lg:flex-col md-lg:justify-start md-lg:items-start w-full gap-4 bg-white p-1 rounded-md'>
                <h2 className='font-bold'>No products available for selected filter</h2>
                </div>
            }
